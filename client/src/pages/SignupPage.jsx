    ...
    <div className="space-y-1.5">
      <div className="flex gap-1">
        {[0,1,2,3].map(i => (
          <div
            key={i}
            className={`h-1 flex-1 rounded-full transition-all duration-300 ${i < score ? colors[score - 1] : "bg-[var(--border-color)]"}`}
          />
        ))}
      </div>
      {score > 0 && (
        <p className={`text-xs font-medium ${score <= 1 ? "text-rose-400" : score === 2 ? "text-orange-400" : score === 3 ? "text-yellow-400" : "text-emerald-400"}`}>
...
function SignupPage() {
  ...
  async function handleGoogleSignup() {
    ...
    try {
      ...
      setError(`Google sign-up failed: ${authError.message}`);
    ...
    <div className="min-h-screen flex bg-[var(--bg-primary)]">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] p-10 relative overflow-hidden"
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute bottom-1/3 -right-12 w-48 h-48 rounded-full bg-white/5" />
          <div className="absolute top-1/2 left-1/3 w-24 h-24 rounded-2xl bg-white/10 -rotate-12" />
          <div className="absolute bottom-12 left-12 w-12 h-12 rounded-full bg-white/20" />
        </div>
        ...
        <div className="relative space-y-6">
          <h2 className="text-3xl font-bold text-white leading-snug">
            Your AI research<br />analyst, available 24/7.
          </h2>
          <div className="space-y-4">
            {[
              { icon: "🤖", text: "7 specialized AI agents analyze every angle" },
              { icon: "📊", text: "Financial, sentiment, moat & risk scoring" },
              { icon: "💡", text: "Explainable decisions — no black boxes" }
            ].map(item => (
              <div key={item.text} className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 text-lg shrink-0">
                  {item.icon}
                </div>
        ...
        <p className="relative text-white/50 text-xs">
          © {new Date().getFullYear()} TradeIntel AI · Built with LangGraph
        </p>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-start justify-center px-6 py-4 overflow-y-auto">
          <motion.div
            ...
            className="w-full max-w-[400px] pb-6"
          >
            <div className="mb-6">
              ...
              <p className="text-sm text-[var(--text-secondary)]">
                Start your AI investment research workspace
              </p>
            </div>
            ...
            {/* Email form */}
            <form id="signup-form" onSubmit={handleEmailSignup} className="flex flex-col gap-3.5">
              <FloatingInput id="signup-fullname" label="Full name *" value={displayName} onChange={e => setDisplayName(e.target.value)} error={!!error} />
              <FloatingInput id="signup-username" label="Username" value={username} onChange={e => setUsername(e.target.value)} />
              <FloatingInput id="signup-phone" label="Contact number" type="tel" value={contactNumber} onChange={e => setContactNumber(e.target.value)} />
              <FloatingInput id="signup-email" label="Email address *" type="email" value={email} onChange={e => setEmail(e.target.value)} error={!!error} />
              ...
              <p className="text-xs text-[var(--text-muted)] text-center leading-relaxed">
                ...
                <span className="text-[var(--color-accent)] cursor-pointer">Terms of Service</span> and{" "}
                <span className="text-[var(--color-accent)] cursor-pointer">Privacy Policy</span>.
              </p>
            </form>

            <div className="mt-5 text-center">
              <span className="text-sm text-[var(--text-secondary)]">Already have an account?{" "}</span>
              <Link to="/login" className="text-sm font-semibold text-[var(--color-accent)] hover:text-[var(--color-accent-hover)] transition-colors">
                Sign in →
              </Link>
            </div>
          </motion.div>
