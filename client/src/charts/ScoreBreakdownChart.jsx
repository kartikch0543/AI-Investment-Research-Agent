...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    const getStatusText = (v) => {
      if (v >= 85) return { text: "Outstanding Performance", color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" };
      if (v >= 70) return { text: "Healthy / Low Risk", color: "text-green-500 bg-green-500/10 border-green-500/20" };
      if (v >= 55) return { text: "Moderate Watch Factor", color: "text-amber-500 bg-amber-500/10 border-amber-500/20" };
      return { text: "High Risk Concern", color: "text-rose-500 bg-rose-500/10 border-rose-500/20" };
    };
    ...
    return (
      <div className="rounded-xl border border-[var(--border-color)] bg-[var(--bg-surface)] p-3 shadow-xl backdrop-blur-md animate-scale-in">
        ...
        <p className="text-xl font-black mt-1 tabular-nums text-[var(--text-primary)]">
          {val}
          <span className="text-xs font-semibold text-[var(--text-muted)] ml-1">/ 100</span>
        </p>
        <span className={`inline-block mt-2 text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full border ${status.color}`}>
          {status.text}
        </span>
      </div>
    );
  ...
  return (
    <GlassPanel className="relative overflow-hidden h-full flex flex-col justify-between">
      <div>
        ...
        {/* Recharts vertical bar chart with gradients & hover states */}
        <div className="h-[210px] w-full pr-2 relative">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <CartesianGrid stroke="var(--border-color)" strokeDasharray="3 3" horizontal={false} opacity={0.6} />
              ...
              <Tooltip 
                ...
                cursor={{ fill: "var(--bg-secondary)", opacity: 0.25 }}
